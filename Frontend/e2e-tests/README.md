...
### Rendering service 2 — module harness

RS2 is a service of its own with its own API and 13 renderer modules
(`projects/rendering-service-lib/src/lib/module/`). Which one runs is decided by two independent
mechanisms, and both are covered in `rs2-modules.spec.ts` (`chromium` only):

1. **backend module** — `POST /public/renderdata` answers with `module` and
   `render.component.html` dispatches on it. In the mock that is one field, picked by node id.
2. **frontend module** — when the backend answers **415** ("no module for this media type"),
   `RenderComponent` falls back to `ModuleInfoService`, which classifies the node purely
   client-side from `ccm:wwwurl`, `mimetype`, `mediatype`, `remote.repository.repositoryType` and
   aspects. That is where the `url` module and its 16 `UrlEmbeddings` come from — no backend at all.

Instead of driving the whole application once per module, the suite mounts the **published web
component** `<edu-sharing-render>` on a bare host page the mock serves at `/rs2-harness?module=…`
(`mock-backend/src/harness.ts`). The component takes the entire node base64-encoded in an
attribute, so a module needs no login, no workspace and no node request — the whole suite runs in
about 8 seconds. It also exercises `isWebComponent: true`, which has its own branches in the
dispatch (PDF through an iframe rather than the pdf viewer, Moodle as a preview image).

Building it is `npm run build:mock-web-component` (target `rendering-service`, configuration
`mock`, ~16s, output `dist-mock-web-components/`).

| Case | Path | Notes |
| --- | --- | --- |
| `IMAGE` | backend | the deterministic base case |
| `VIDEO` | backend | two links of different height, so the resolution menu appears |
| `AUDIO` | backend | |
| `DDB` | backend, **job path** | `DdbComponent` builds its url from `items[0].additionalData`, and the no-job path hardcodes that to `undefined` — so this case needs a `jobId` and is the only one that exercises the polling branch |
| `ERROR` | backend | |
| `default` | frontend (415) | |
| `url` / YOUTUBE, VIMEO | frontend (415) | rendered as the **GDPR consent overlay** |
| `url` / LINK | frontend (415) | preview image plus link button |
| `url` / AUDIO | frontend (415) | player pointing at the node's own url |

Why these and not the others:

* **`AssetLinkPipe` rewrites every asset link to the current RS2 root**, cutting it at `/public`.
  The host in the link is therefore irrelevant and all media, images and iframes stay on the mock's
  own origin — which is what makes the modules renderable offline at all.
* Video and audio get a **real** media file: `mock-backend/assets/media/sample.wav`, generated by
  hand because the build box has no ffmpeg. A player with a broken source renders differently from
  a working one, so a placeholder would have been worse than useless.
* The **GDPR consent state** is reachable because `GdprService` reads nothing but the client config
  (`gdpr.enabled` + `entry[].regex`). It is both the state a privacy-compliant deployment shows and
  the only one that renders without reaching YouTube or Vimeo.
* **Not covered**: everything that needs a third-party host — the `url` embeddings *after* consent,
  SODIX/OMEGA — and the iframe modules H5P, EDUHTML, MOODLE and BINDER, which are same-origin and
  therefore possible, just not done yet.
* **No image-embedding case**: that branch of the `url` module binds `[ngSrc]` without the
  `width`/`height` (or `fill`) `NgOptimizedImage` requires. It throws NG02954 and renders an empty
  box — an application bug, and a test would only pin it into a baseline.
